...
Updated on Thu Jan 8 2021

@authors: Lantian ZHANG <peter.lantian.zhang@outlook.com>

- Important features: Features who contribute the most in pusing the total score above a threshold.
- Change attribution: The score changes between time t and t-1, sorted by the difference descendingly
- Highest-score interval: Features where the instance is in the interval with highest score, sorted by score descendingly
...
def _instance_important_features(scores,feature_names,threshold):
    ...
    in pusing the total score above a threshold.
    ...
    threshold =  max_value if max_value<threshold else threshold
    tem = tem.iloc[0:tem.index[tem['cum_score']<threshold].max()+1,:]
...
def important_features(scored_df,feature_names,col_totalscore='TotalScore',threshold_method=0.8, bins=None):
    ...
    # Filter top n important features
        ...
        raise TypeError('unsupported threshold_method')
